...
In Computer Science, it is very interesting to analyze the cost of an algorithm in the worst case.

[Big O notation](https://en.wikipedia.org/wiki/Big_O_notation) is one of the most fundamental tools
to measure the cost of an algorithm given a large size of input.
...
Typically, the complexity of an algorithm is determined by the dominant time-consuming section of the function
...
because time complexity ignores coefficient, constant, and smaller terms in the polynomial.

$$
T(n + n^2 + \frac{n^4}{5}) \in O(n^4)
$$

**For a recursive function where it invokes itself**, a [decision tree](https://en.wikipedia.org/wiki/Decision_tree)
that represents subsequent invocations according to inputs can help us account for time complexity.

Most of recursive functions branching several times are more likely to take exponential time if there is no pruning.
...
From a first glance, it is easy to understand that function `fibonacci` branches twice in every invocation
...
In the decision tree, a tree height means the number of consecutive invocations in the worst case
and is the same as the size of call stacks when it reaches the base case.
